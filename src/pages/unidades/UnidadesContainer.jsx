...
const UnidadesContainer = () => {
  ...
  const { data: dataUnidades, loading: loadingUnidades } =
    useFetch(`/unidades`);
    ...
    <Layout>
      ...
      <section className="h-screen">
        ...
        {loading ? (
          ...
          <Slider
            data={data.filter((item) => item.category == 4)}
            autoplay={false}
            indicators={false}
          />
        )}
              ...
              {dataUnidades.map((item) => (
                <article className="bg-secondary z-30 mx-2" key={item.id}>
                  ...
                  <div className="text-white p-8 text-sm">
                    <h2 className="uppercase font-extra text-3xl pb-2 border-white border-b-2 mb-4">
                      {item.title}
                    </h2>
                    <p className="mb-8">
                      <TextoHTML
                        html={lan === "es" ? item.text : item.text_eng}
                      />
                    </p>
                    <a
                      href={item.file}
                      target="_blank"
                      rel="noreferrer"
                      className="text-black bg-primary px-10 py-3 font-bold text-sm inline-block bg-white-hover hover:shadow-lg"
                    >
          ...
          data
            ...
            .map((item) => (
              <div className="w-full aspect-video block" key={item.id}>
                <ImageComponent
                  src={item.image}
                  alt="Imagen pie de página de Unidades"
                />
              </div>
            ))
        )}
